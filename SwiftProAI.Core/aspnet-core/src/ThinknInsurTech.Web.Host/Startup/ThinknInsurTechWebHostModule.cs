...
using ThinknInsurTech.Authorization.Users.Password;
...
using ThinknInsurTech.MultiTenancy.Subscription;
using ThinknInsurTech.Web.Startup.ExternalLoginInfoProviders;

namespace ThinknInsurTech.Web.Startup
{
    [DependsOn(
        typeof(ThinknInsurTechWebCoreModule)
    )]
    public class ThinknInsurTechWebHostModule : AbpModule
    {
        private readonly IWebHostEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public ThinknInsurTechWebHostModule(
        ...
        public override void PreInitialize()
        {
            ...
            IocManager.Register<IIntegrationService, IntegrationService>(DependencyLifeStyle.Transient);

            var isOCREnabled = Convert.ToBoolean(_appConfiguration["OCR:IsOCREnabled"]);

            if (isOCREnabled){
                IocManager.Register<IOCRPromptService, OCRPromptService>(DependencyLifeStyle.Transient);
            }
            else
            {
                IocManager.Register<IOCRPromptService, MockOCRPromptService>(DependencyLifeStyle.Transient);
            }
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(ThinknInsurTechWebHostModule).GetAssembly());
        }
