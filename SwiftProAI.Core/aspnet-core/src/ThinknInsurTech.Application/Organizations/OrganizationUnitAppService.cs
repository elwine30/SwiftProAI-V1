...
namespace ThinknInsurTech.Organizations
{
    ...
    public class OrganizationUnitAppService : ThinknInsurTechAppServiceBase, IOrganizationUnitAppService
    {
        ...
        public async Task<OrganizationUnitDto> CreateOrganizationUnit(CreateOrganizationUnitInput input)
        {
            using (_unitOfWorkManager.Current.DisableFilter(ThinknInsurTechDataFilters.HaveOrganizationUnit))
            {
                ...
                if (input.CompanyType != null && input.CompanyType != "")
                {
                    var roleAssigned = _roleRepository.FirstOrDefault(f => f.Name == input.CompanyType);
                    var orgRole = new OrganizationUnitRole(AbpSession.TenantId, roleAssigned.Id, organizationUnit.Id);
