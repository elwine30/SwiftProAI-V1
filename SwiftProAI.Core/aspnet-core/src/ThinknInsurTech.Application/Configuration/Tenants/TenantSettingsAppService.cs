...
using ThinknInsurTech.Security;
using ThinknInsurTech.Storage;
using ThinknInsurTech.Timing;

namespace ThinknInsurTech.Configuration.Tenants
{
    ...
    public class TenantSettingsAppService : SettingsAppServiceBase, ITenantSettingsAppService
    {
        ...
        private async Task<TenantBillingSettingsEditDto> GetBillingSettingsAsync()
        {
            ...
            {
                CompanyRegisterId = await SettingManager.GetSettingValueAsync(AppSettings.TenantManagement.BillingCompanyRegisterId),
                ...
                CompanyTelNo = await SettingManager.GetSettingValueAsync(AppSettings.TenantManagement.BillingCompanyTelNo),
            };
        }
        ...
        {
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(),
                AppSettings.TenantManagement.BillingCompanyRegisterId, input.CompanyRegisterId);
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(),
            ...
            await SettingManager.ChangeSettingForTenantAsync(AbpSession.GetTenantId(),
                AppSettings.TenantManagement.BillingCompanyTelNo, input.CompanyTelNo);
        }
        ...
        {
            if (_multiTenancyConfig.IsEnabled && !ThinknInsurTechConsts.AllowTenantsToChangeEmailSettings)
            {
                return;
            }
