...
using ThinknInsurTech.Test.Base.TestData;
...
namespace ThinknInsurTech.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="ThinknInsurTechDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(ThinknInsurTechDbContext context)
            {
        ...
        protected void UsingDbContext(Action<ThinknInsurTechDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ThinknInsurTechDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<ThinknInsurTechDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<ThinknInsurTechDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ThinknInsurTechDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ThinknInsurTechDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ThinknInsurTechDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ThinknInsurTechDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<ThinknInsurTechDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<ThinknInsurTechDbContext, Task<TResult>> func)
        {
            TResult result;
