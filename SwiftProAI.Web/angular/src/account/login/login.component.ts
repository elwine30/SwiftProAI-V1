...
import { AccountServiceProxy, IsTenantAvailableInput, IsTenantAvailableOutput, SessionServiceProxy, TenantAvailabilityState, UpdateUserSignInTokenOutput } from '@shared/service-proxies/service-proxies';
...
export class LoginComponent extends AppComponentBase implements OnInit, AfterViewInit {
    ...
    constructor(
        ...
        const internalSegment = urlSegments.find(segment => segment.path === 'internal') == undefined ? null : urlSegments.find(segment => segment.path === 'internal').path;
        if(internalSegment != null && internalSegment == 'internal') {
            ...
            .subscribe((result: IsTenantAvailableOutput) => {
                switch (result.state) {
                    ...
                    case TenantAvailabilityState.InActive:
                        this.message.warn(this.l('TenantIsNotActive', input.tenancyName));
                        break;
                    case TenantAvailabilityState.NotFound: //NotFound
                        this.message.warn(this.l('ThereIsNoTenantDefinedWithName{0}', input.tenancyName));
                        break;
                }
            });
        }
